...
class Storage<T> {
  ...
  insert(item: StorageProps<T>): T[] {
    const key: keyof typeof item = Object.entries(item).map(([key]) => key)[0]
  ...
  obtain(item: StorageProps<Partial<T>>): T[] {
    const key = Object.entries(item).map(([key]) => key)[0] as string

    const baseObjectFind = item[key]

    const keyOutFindInItems = Object.entries(baseObjectFind).map(
      ([key]) => key,
    )[0] as keyof typeof baseObjectFind
    ...
    if (valueToFind) {
      return jsonItems?.find(
        (item: T) => item[keyOutFindInItems] === valueToFind,
      )
    }
  ...
  remove(item: StorageProps<Partial<T>>): T[] {
    ...
    const keyItemOutIndexToFindObjectRemove = Object.entries(itemToRemove).map(
      ([key]) => key,
    )[0] as keyof typeof itemToRemove

    const valueItemOutIndexToFindObjectRemove = (item as Record<string, T>)[
      key
    ][keyItemOutIndexToFindObjectRemove]

    const newItems = jsonItems?.filter(
      (item: T) =>
        item[keyItemOutIndexToFindObjectRemove] !==
        valueItemOutIndexToFindObjectRemove,
    )
